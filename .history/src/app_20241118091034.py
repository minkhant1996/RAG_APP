...
async def chatbot_endpoint(request: chatbot_api):
    ...
    user_data = RedisDB_Manager.add_chat_history(redis_key, user_data, user_query=user_input)
    
    print(user_data)
    if not user_input:
        raise HTTPException(status_code=400, detail="Input text cannot be empty.")

    # Example: Reverse the input text for the response
    response_text = f"You said: {user_input[::-1]}"
